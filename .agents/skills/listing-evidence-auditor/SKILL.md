...
description: Use when independently reconciling final listing asset claims against real-file identity, approval provenance, visual role evidence, and required asset-set completeness.
---
...
Reject unsupported/invalid image signatures, extension/signature mismatch, path escape, missing files, zero/invalid dimensions, and malformed identity records.
...
A same-name or same-Asset-ID replacement with different bytes does not inherit prior approval.

Exact recovery is allowed only when the physical hash, role, and scope match the previously locked exact asset.
...
Every required slot must point to a final-consumable exact asset with matching approved scope. One invalidated, missing, unverified, or out-of-scope required member blocks the asset-set gate.
...
The auditor **must not repair** a failing asset or silently change its role, source, approval, or scope. Return evidence findings to the owning Planning / Production / Hardening plane.
